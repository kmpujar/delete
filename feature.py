...
class Features:
    ...
    def jac_sim(self,s,p,o,body):
        scores=[]
        claim=s+p+o
        lsen=tokenize.sent_tokenize(body)
        for l in lsen:
            scores.append(self.get_jaccard_simi(claim,l))
        return max(scores)

    def get_cosine_similarity(slef,str1,str2):
    ...
    def get_semantic_sim(self,s,p,o,body):
        scores=[]
        claim=s+p+o
        lsen=tokenize.sent_tokenize(body)
        for l in lsen:
            scores.append(nlp(l).similarity(nlp(claim)))
    ...
    def extract_features(self, doc, subject, predicate, object):
        ...
        vec.append(self.jac_sim(subj,pred,obj,body))
        vec.append(self.get_cosine_sim(subj,pred,obj,body))
        vec.append(self.get_semantic_sim(subj,pred,obj,body))
        return vec
